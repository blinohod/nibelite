...
class CMS {
	...
	// HANDLE method. This is an CMS tasks dispatcher.
	// Our task-handles format is: "<cms prefix>-<cms task>"
	// At example, "insert vendor" handle is: "vendor-insert"

	function handle ($cmd = 'do') {
	...
	function redir ($action,$st,$params=array(),$anchor='') {
		$query = "Location: {$_SERVER['SCRIPT_NAME']}?do={$this->prefix}-{$action}&status=".urlencode($st);
		if($params)
				...
				foreach($this->fields as $field=>$contents)
					if(is_array($contents)){
						$hash["[{$field}]"] = selector($field.'" style="width:240px',
							...
							0,translate('none'));
						$hash["({$field})"] = isset($contents[$rec[$field]]) ? $contents[$rec[$field]] : 'NO DATA '.$rec[$field];
					}
